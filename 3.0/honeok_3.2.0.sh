...
system_info() {
    ...
    if [ "$country" == "CN" ];then
        for api in "$taobao_timeapi" "$suning_timeapi" "$ddnspod_timeapi"; do
            # 判断时间是否有效，假设有效时间格式是 "YYYY-MM-DD HH:MM:SS"
            if [[ -n "$api" && "$api" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$ ]]; then
                china_time="$api"
                break
            fi
        done
